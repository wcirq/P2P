...
myPeerBPort1 = message[0]
signature = str(message[1])
print('got myPeer: ', myPeerBPort1)
...
def sendToMyPeer():
    # 发送包含签名的连接请求
    global peerConnected, myPeerBPort1
    while True:
        s.sendto(signature.encode(), myPeerBPort1)
        if peerConnected:
            break
        time.sleep(1)
    ...
    # 发送聊天信息
    while True:
        send_text = input(f"(to {myPeerBPort1}) 我方发送：")
        if os.path.exists(send_text):
            # 是文件就发送文件
            file_path = send_text
            send_file(myPeerBPort1, file_path)
        else:
            s.sendto(send_text.encode(), myPeerBPort1)


def recFromMyPeer():
    # 接收请求并验证签名or接收聊天信息
    global peerConnected, stop_receive
    while True:
        ...
        if message == signature:
            ...
            if message[:3] == "[0]":
                ...
                s.sendto('ok'.encode('utf-8'), myPeerBPort1)
                print(f"开始接收文件2 {file_name}")
                receive_file(myPeerBPort1, file_name)
                stop_receive = False
        print(end="")
